function test_make_vcov
    ...
    % Ensure that the minimum eigenvalue of test_pd2 is exactly zero.
    eigenvalues_pd2       = eig(test_pd2);
    [eigenvectors_pd2, ~] = eig(test_pd2); 
    reduced_eigenvalues   = eigenvalues_pd2 - min(eigenvalues_pd2);
    eigenvalue_diag       = diag(reduced_eigenvalues);
    test_pd2              = eigenvectors_pd2 * eigenvalue_diag * (eigenvectors_pd2)^(-1);
