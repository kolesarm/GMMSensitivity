...
methods
    ...
    function obj = AddData(obj, varargin)
        ...
        for j = 2:nargin
            % We use the inputname() function to recover the names of
            % the variables passed to AddData() through varargin.
            variable_names{j - 1} = inputname(j);
        end
...
methods (Static, Hidden, Access = protected)
    function out = ParseInputList(list, variable_names)
    ...
    % for each field.
        ...
        % Create a cell of variable names if none is provided
        if nargin < 2
            variable_names = cell(size(list));
            for j = 1:length(list)
                variable_names{j} = ['var_' num2str(j)];
            end
        end
        
        for i = 1:length(list)
            if isnumeric(list{i}) || iscellstr(list{i})
                out = [out {{list{i}, variable_names{i}}}]; 
